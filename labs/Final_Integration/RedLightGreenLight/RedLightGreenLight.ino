...
#define frontDist 190 
#define alleyDist 230
  ...
  switch(recentDirection){
    ...
    if (xpos-1 >=0){
      if (Maze[xpos-1][ypos].visited) {
        Maze[xpos][ypos].west =1;
        if (!Maze[xpos-1][ypos].cleared){
          if(Maze[xpos-1][ypos].north && Maze[xpos-1][ypos].east && Maze[xpos-1][ypos].south && Maze[xpos-1][ypos].west){
            Maze[xpos-1][ypos].cleared=1;
            openNodes--;
          }
        }
      }
    ...
    if (ypos+1 <= 8){
      if (Maze[xpos][ypos+1].visited) {
        Maze[xpos][ypos].north =1;
        if (!Maze[xpos-1][ypos].cleared){
          if(Maze[xpos][ypos+1].north && Maze[xpos][ypos+1].east && Maze[xpos][ypos+1].south && Maze[xpos][ypos+1].west){
              Maze[xpos][ypos+1].cleared=1;
              openNodes--;
          }
        }
      }
    ...
    if(Maze[xpos][ypos].south==0){
    ...
    else if(Maze[xpos][ypos].north==0){
      nextDirection = North;
      Maze[xpos][ypos].north=1;
    }
      ...
      if (!Maze[xpos][ypos].cleared){
        Maze[xpos][ypos].cleared=1;
        openNodes--;
      }
      nextDirection = (Maze[xpos][ypos].direction+2)%4;
    }
    //Serial.print("OpenNodes:");
    //Serial.println(openNodes);

//    if (openNodes==0 && xpos!=0 && ypos!=0){
...
void setup() {
  //Store original values to reset post FFT
  adcsra = ADCSRA; 
  admux = ADMUX; 
  didr0 = DIDR0; 
...
//  }      
  ...
  else if(start_sound == 0 && !start){
    left.detach();
    right.detach();
    Serial.println("FFT BEGIN");
      while(!start_sound) { // reduces jitter
        ...
        //  Serial.println(out + fht_log_out[i] + " Bin:" +i); //send out data to serial
        //}
        if (((fht_log_out[6]>fht_log_out[7]) && (fht_log_out[6] > 120) && (fht_log_out[7] > 110) && (fht_log_out[5] < 100))||(digitalRead(2) == 1)){ //Turn LED on when bin 6 has a high enough magnitude - about 950Hz
          Serial.println("FFT ENDED");
          start_sound = 1;
          ADCSRA = adcsra; 
          ADMUX = admux; 
          DIDR0 = didr0;
