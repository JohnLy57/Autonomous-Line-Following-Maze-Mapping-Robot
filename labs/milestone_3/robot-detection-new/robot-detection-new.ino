/**
 * A demonstration for robot detection
*/
...
void checkWall()
{
  ...
  checkDistSensors();
  // all walls
  if ((distRight < distHigh && distRight > distLow) && (distFront > 235) && (distBack < distHigh && distBack > distLow))
  {
    pivot180();
  }
...
//IR robot detection sensor threshold
#define robotDetectThreshold 700 // if >700, there's robot; if < then no

/** Returns 1 if any robot is detected in any direction (front, left, or right),
 * 0 if none detected
 */ 
int robotDetect() {
  if ( (irFront >= robotDetectThreshold) || (irLeft >= robotDetectThreshold) || 
    (irRight >= robotDetectThreshold) || (irBack >= robotDetectThreshold) ) {
      return 1;
    } else {
      return 0;
    }
...
/** If other robots detected, then stop and wait until they go out of range
 * 
 * To use this function, simply insert it before any robot movement function
 */
void robotWait() {
  while (robotDetect() == 1); //todo: maybe light up LED indicator during waiting
  return;
}
