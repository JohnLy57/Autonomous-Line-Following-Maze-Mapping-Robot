...
/* Detect other robots and react accordingly */
void detectRobot() {
  // TODO

  int irFrontOld = irFront;
  int irBackOld = irBack;
  int irLeftOld = irLeft;
  int irRightOld = irRight;
  checkIR();
  // there can be multiple robots inbound
  int isRobotFront = (irFront - irFrontOld >= irDiff);
  int isRobotBack = (irBack- irBackOld >= irDiff);
  int isRobotLeft = (irLeft - irLeftOld >= irDiff);
  int isRobotRight = (irRight - irRightOld >= irDiff);

  // if front detects robot
  if (isRobotFront) {
    // do something
  }
  // if back detects robot
  else if (isRobotBack) {
    // do something
  }
  // if left detects robot
  else if (isRobotLeft) {
    // do something
  }
  // if right detects robot
  else if (isRobotRight) {
    // do something
  }
  //else no robot; go forward
  else { 
    pd_followLine();
  }
}
